...
const TIER_BODY: Record<90 | 70 | 50, string> = {
  90: "Lower variance baskets with wider senior coverage.",
  70: "Balanced baskets with visible mezzanine and junior pricing.",
  50: "Low probability baskets with wider junior exposure.",
};
    ...
    <>
      ...
      <PageFrame wide>
        ...
        <div className="risk-shell">
          ...
          {state.status === "error" && <EmptyState title="Could not load baskets" subtitle={state.error} />}
          {state.status === "ok" && (
              ...
              {([90, 70, 50] as const).map((tier) => {
                ...
                return (
                  <TierGroup
                    ...
                    tier={tier}
                    baskets={baskets}
                    onClick={(basket) => router.push(`/app/tranche/${basket.id}`)}
                  />
                );
              })}
            </div>
...
function TierGroup({
  ...
  tier: 90 | 70 | 50;
  baskets: LiveBasket[];
  onClick: (basket: LiveBasket) => void;
}) {
  const color = tc(tier);
  return (
    <section className="risk-tier">
      ...
      <div className="risk-grid">
        {baskets.map((basket) => (
          <BasketTrancheCard key={basket.id} basket={basket} onClick={() => onClick(basket)} />
        ))}
      </div>
    </section>
  );
  ...
  return (
    <button className="risk-card" type="button" onClick={onClick}>
      ...
      <div className="risk-slice-list">
        {quotes.map((quote) => (
          <TrancheRow key={quote.kind} quote={quote} />
        ))}
      </div>
    </button>
  );
...
function DistributionBand({
  ...
  const senior = quotes.find((quote) => quote.kind === "senior");
  const mezzanine = quotes.find((quote) => quote.kind === "mezzanine");
  const junior = quotes.find((quote) => quote.kind === "junior");
  if (!senior || !mezzanine || !junior) return null;

  const navPct = Math.max(0, Math.min(100, nav * 100));
  const segments = [senior, mezzanine, junior].map((quote) => ({
    pct: quote.notionalShare * 100,
    color: trancheColor(quote.kind),
    title: quote.kind,
  }));
...
const RISK_CSS = `
  ...
  .risk-card { width: 100%; appearance: none; border: 0.5px solid ${C.border}; background: ${C.surface}; border-radius: 8px; padding: 13px; text-align: left; cursor: pointer; transition: background 0.14s ${EASE}, border-color 0.14s ${EASE}, transform 0.14s ${EASE}; }
  ...
  .risk-card-title strong { color: ${C.textPrimary}; font-family: ${FD}; font-size: 13.5px; font-weight: 620; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; }
  ...
  .risk-day { height: 28px; min-width: 50px; border-radius: 999px; border: 0.5px solid ${C.border}; color: ${C.textPrimary}; background: ${C.card}; display: inline-flex; align-items: center; justify-content: center; font-family: ${FD}; font-size: 12px; font-weight: 620; }
  ...
  .risk-slice-list { display: grid; border: 0.5px solid ${C.border}; border-radius: 8px; overflow: hidden; }
  ...
  .risk-slice-row span { display: block; font-family: ${FM}; font-size: 9.5px; letter-spacing: 0.13em; text-transform: uppercase; font-weight: 640; }
  ...
  .risk-slice-row b { font-family: ${FM}; font-size: 10.5px; font-weight: 640; text-align: right; font-variant-numeric: tabular-nums; }
  .risk-slice-row b em { margin-left: 3px; color: ${C.textMuted}; font-size: 9.5px; }
  .risk-loading-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: 10px; }
  .risk-loading-grid div { height: 230px; border: 0.5px solid ${C.border}; background: ${C.card}; border-radius: 8px; opacity: 0.6; }
  ...
  @media (max-width: 1120px) {
    .risk-grid, .risk-loading-grid { grid-template-columns: repeat(2, minmax(0, 1fr)); }
  }
  @media (max-width: 720px) {
    .risk-grid, .risk-loading-grid { grid-template-columns: 1fr; }
    .risk-tier-head { align-items: flex-start; flex-direction: column; gap: 8px; }
    .risk-slice-row { grid-template-columns: minmax(0, 1fr) 78px 84px; }
  }
`;
