 ...
 *                           (narrow ATM = high hit-rate/low multiple; wide = convex).
...
export async function quotePpn(args: {
  ...
  return {
    ...
    total_max_payout_raw: (floorRaw + BigInt(strip.total_max_payout_raw)).toString(),
    dusdc_decimals: 6,
  };
}
...
const TRANCHE_DEFS: Array<{ tranche: TrancheProfile['tranche']; mult: number; label: string }> = [
  { tranche: 'senior', mult: 0.5, label: 'Senior — tight ATM, high hit-rate, low multiple' },
  { tranche: 'mezz', mult: 1.0, label: 'Mezzanine — balanced width' },
  { tranche: 'junior', mult: 2.0, label: 'Junior — wide/convex, low hit-rate, high multiple' },
];
...
export async function quoteTranches(args: {
  ...
  for (const d of TRANCHE_DEFS) {
    const strip = await previewStrip({
      oracle: args.oracle, muRaw: args.forwardRaw, sigmaRaw: Math.round(args.sigmaRaw * d.mult), n: args.n, budgetRaw: args.budgetRaw, sender: args.sender,
    });
...
export const DEEPBOOK_BASKETS: BasketRecipe[] = [
  ...
  { id: 'btc-convex', name: 'BTC Wide', description: 'Wide, convex distribution — pays on a big move either way.', sigma_pct: 0.010, n: 8 },
];
